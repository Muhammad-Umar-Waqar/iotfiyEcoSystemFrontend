...
/**
 * Normalizes GET /dashboard/all-orgWithDetails response.
 * Returns flat array for the top-level Organizations view.
 */
function normalizeAllOrgs(data) {
  return (data.organizations || []).map(o => ({
    id:      o.id,
    name:    o.name,
    owner:   o.owner?.name  || 'N/A',
...
/**
 * Normalizes GET /dashboard/all-venueWithDevices response.
 * Returns flat array for the top-level Venues view.
 */
function normalizeAllVenues(data) {
  return (data.venues || []).map(v => ({
    id:      v.id,
    name:    v.name,
    org:     v.organization?.name || 'N/A',
    devices: v.totalDevices ?? 0,
  }));
}

/**
 * Normalizes GET /dashboard/all-devices response.
 * Returns flat array for the top-level Devices view.
 */
function normalizeAllDevices(data) {
...
//       whiteSpace: 'nowrap',
//     }}>
//       {colors.dot && <span style={{ width: 5, height: 5, borderRadius: '50%', background: colors.dot, flexShrink: 0 }} />}
//       {label}
//     </span>
//   );
// }
...
function ManagersTable({ managers, onRowClick, onStatusClick, updatingManagerId }) {
  ...
  <button
    ...
    disabled={updatingManagerId === m.id}
    style={{
      border: 'none',
      background: 'transparent',
      padding: 0,
      cursor: updatingManagerId === m.id ? 'wait' : 'pointer',
      width: '100%',
      textAlign: 'left',
    }}
...
export default function AdminDashboard() {
  ...
  /* ── API state: all organizations (top-level view) ── */
  const [allOrgs,        setAllOrgs]        = useState([]);
  const [allOrgsLoading, setAllOrgsLoading] = useState(false);
  const [allOrgsError,   setAllOrgsError]   = useState(null);
  const [allOrgsTotal,   setAllOrgsTotal]   = useState(0);

  /* ── API state: all venues (top-level view) ── */
  const [allVenues,        setAllVenues]        = useState([]);
  const [allVenuesLoading, setAllVenuesLoading] = useState(false);
  const [allVenuesError,   setAllVenuesError]   = useState(null);
  const [allVenuesTotal,   setAllVenuesTotal]   = useState(0);

  /* ── API state: all devices (top-level view) ── */
  const [allDevices,        setAllDevices]        = useState([]);
  const [allDevicesLoading, setAllDevicesLoading] = useState(false);
  const [allDevicesError,   setAllDevicesError]   = useState(null);
  const [allDevicesStats,   setAllDevicesStats]   = useState({ total: 0, online: 0, offline: 0 });

  const [selectedManagerMeta, setSelectedManagerMeta] = useState(null);
const [updatingManagerId, setUpdatingManagerId] = useState(null);

const toggleManagerStatus = useCallback(async (manager) => {
  const isCurrentlyActive = manager.status === 'Active';
  const nextIsActive = !isCurrentlyActive;

  const result = await Swal.fire({
    title: nextIsActive ? 'Activate manager?' : 'Deactivate manager?',
    html: nextIsActive
      ? 'This manager and their sub-users will become active again.'
      : 'This manager will become inactive, and all of their sub-users will also be made inactive.',
    input: nextIsActive ? undefined : 'textarea',
    inputPlaceholder: 'Enter suspension reason',
    inputLabel: nextIsActive ? undefined : 'Suspension reason',
    showCancelButton: true,
    confirmButtonText: nextIsActive ? 'Yes, activate' : 'Yes, deactivate',
    cancelButtonText: 'Cancel',
    reverseButtons: true,
    preConfirm: (value) => {
      if (!nextIsActive && (!value || !value.trim())) {
  ...
  const suspensionReason = nextIsActive ? null : result.value.trim();

  setUpdatingManagerId(manager.id);

  try {
    const res = await fetch(`${BASE}/user/suspend/${manager.id}`, {
      method: 'PUT',
      credentials: 'include',
      headers: authHeaders(),
      body: JSON.stringify({
    ...
    setManagersStats(prev => ({
  ...prev,
  active: nextIsActive
    ? prev.active + 1
    : Math.max(0, prev.active - 1),
  inactive: nextIsActive
    ...
    setManagers(prev =>
      prev.map(m =>
        m.id === manager.id ? { ...m, status: nextStatus } : m
      )
    );

    setSelectedManagerMeta(prev =>
      prev && prev.id === manager.id ? { ...prev, status: nextStatus } : prev
    );

    setManagerDetail(prev =>
      prev && prev.manager?.id === manager.id
        ? {
            ...prev,
            manager: {
              ...prev.manager,
    ...
    Swal.fire({
      icon: 'success',
      title: 'Updated',
      text: data.message || `Manager has been ${nextIsActive ? 'activated' : 'deactivated'}.`,
      timer: 1800,
    ...
    Swal.fire({
      icon: 'error',
      title: 'Update failed',
      text: err.message || 'Could not update manager status',
    });
  } finally {
  ...
  const fetchAllOrgs = useCallback(async () => {
    ...
    try {
      const res  = await fetch(`${BASE}/dashboard/all-orgWithDetails`, { credentials: 'include', headers: authHeaders() });
      const data = await res.json();
      if (!res.ok || !data.success) throw new Error(data.message || 'Failed to load organizations');
      setAllOrgsTotal(data.total ?? 0);
      setAllOrgs(normalizeAllOrgs(data));
    } catch (err) {
      console.error('fetchAllOrgs:', err);
      setAllOrgsError(err.message);
    } finally {
  ...
  const fetchAllVenues = useCallback(async () => {
    setAllVenuesLoading(true);
    setAllVenuesError(null);
    try {
      const res  = await fetch(`${BASE}/dashboard/all-venueWithDevices`, { credentials: 'include', headers: authHeaders() });
      const data = await res.json();
      if (!res.ok || !data.success) throw new Error(data.message || 'Failed to load venues');
      setAllVenuesTotal(data.total ?? 0);
      setAllVenues(normalizeAllVenues(data));
    } catch (err) {
      console.error('fetchAllVenues:', err);
      setAllVenuesError(err.message);
    } finally {
      setAllVenuesLoading(false);
    }
  }, []);
  ...
  const fetchAllDevices = useCallback(async () => {
    ...
    try {
      const res  = await fetch(`${BASE}/dashboard/all-devices`, { credentials: 'include', headers: authHeaders() });
      const data = await res.json();
      if (!res.ok || !data.success) throw new Error(data.message || 'Failed to load devices');
      const normalized = normalizeAllDevices(data);
      setAllDevicesStats({
        total:   data.total ?? 0,
      ...
      setAllDevicesError(err.message);
    } finally {
      setAllDevicesLoading(false);
    }
  }, []);

  /* ── Lazy-load flat views when their tab is first opened ── */
  useEffect(() => {
    if (view === 'orgs'    && allOrgs.length    === 0 && !allOrgsLoading    && !allOrgsError)    fetchAllOrgs();
    if (view === 'venues'  && allVenues.length  === 0 && !allVenuesLoading  && !allVenuesError)  fetchAllVenues();
    if (view === 'devices' && allDevices.length === 0 && !allDevicesLoading && !allDevicesError) fetchAllDevices();
  ...
  /* ── Current detail objects (derived) ── */
  // const currentManager  = managerDetail?.manager  ?? null;
  const currentManager = managerDetail?.manager
    ? {
        ...managerDetail.manager,
        planLimits: selectedManagerMeta?.planLimits,
        plan: selectedManagerMeta?.plan,
        planType: selectedManagerMeta?.planType,
      }
    : null;
  const currentUser     = userDetail?.user        ?? null;
          ...
          <>
            ...
            {!managersLoading && !managersError && (
              ...
              <ManagersTable
                ...
                onStatusClick={toggleManagerStatus}
            ...
            {!allOrgsLoading && !allOrgsError && (
              <DataTable
                columns={[
                  ...
                  { key: 'owner', header: 'Manager', render: r => (
                    <div>
                      <div style={{ fontSize: 13, color: C.text }}>{r.owner}</div>
                      <div style={{ fontSize: 11, color: C.textSoft, marginTop: 1 }}>{r.ownerEmail}</div>
                    </div>
                  )},
                  { key: 'venues',  header: 'Venues',  render: r => <span style={{ fontWeight: 500, color: C.text }}>{r.venues}</span>  },
                  { key: 'devices', header: 'Devices', render: r => <span style={{ fontWeight: 500, color: C.text }}>{r.devices}</span> },
