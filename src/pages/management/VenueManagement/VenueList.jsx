...
const VenueList = ({ onVenueSelect, selectedVenue }) => {
  const dispatch = useDispatch();
  const { Venues = [], loading = false, error = null, venuesByOrg = {} } =
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      await dispatch(deleteVenue(id)).unwrap();
      dispatch(fetchAllVenues()).catch(() => {});
      if (orgFilter) dispatch(fetchVenuesByOrganization(orgFilter)).catch(() => {});
      window.dispatchEvent(new Event("venue:updated"));
      ...
      Swal.fire({
        title: "Error!",
        text: err?.toString() || "Failed to delete venue",
        icon: "error",
      });
    }
  };
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      Swal.fire({
        title: "Error!",
        text: err?.toString() || "Failed to update venue",
        icon: "error",
      });
    }
  };
  ...
  const getDisplayedVenues = () => {
    ...
    const cached = Array.isArray(venuesByOrg[orgFilter])
      ? venuesByOrg[orgFilter]
      : null;

    const orgVenues =
      cached ||
      (Venues || []).filter(
        (v) => String(v.organization?._id ?? v.organization ?? "") === String(orgFilter)
      );
    ...
    const userAssignedVenueIds = (user?.venues || []).map((v) =>
      String(v.venueId?._id || v.venueId)
    );
